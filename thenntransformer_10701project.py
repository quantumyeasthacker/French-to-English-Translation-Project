...
data = pd.read_csv(file_path, nrows = int(110_000))
...
model_name = 'nn_transformer_new_tokenizer110'
...
pairs = list(zip (data['fr'], data['en']))
...
def eval_epoch(model):
    ...
    model.eval()
    losses = 0
    eval_dataloader = DataLoader(pairs, batch_size=BATCH_SIZE, collate_fn=collate_fn)
...
for epoch in range(1, NUM_EPOCHS+1):
    ...
    end_time = timer()
    val_loss = eval_epoch(transformer)
    save_loss_to_csv(train_loss, 'test_losses.csv')
...
score = sacreblue.compute(predictions=[translate(transformer, "Bonjour le monde. Je suis fatigué mais vivant.")],references=[["Bonjour le monde. Je suis fatigué mais vivant."]])
# print(f'the BLUE score is: {score}')
score['score']

data = pd.read_csv(file_path, nrows = int(1_000), skiprows=int(110_000), names=['en','fr'])
pairs = list(zip(data['fr'], data['en']))
BATCH_SIZE = 1

def evaluate_bleu(model, pairs):

    bleu_score = []

    for src_text, tgt_text in pairs:
        score = sacreblue.compute(predictions=[translate(model, src_text)],
                                             references=[[tgt_text]])
        bleu_score.append(score['score'])
    return bleu_score

bleu_scores = evaluate_bleu(transformer, pairs)
